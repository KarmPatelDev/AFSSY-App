...
export default EditPost = () => {
    ...
    const id = post.id;
    const [title, setTitle] = useState(post._data.title);
    const [description, setDescription] = useState(post._data.description);
    const [postUrl, setPostUrl] = useState(post._data.postUrl);
    ...
    const handleSubmit = async () => {
        try{
            if(!title || !description || !postUrl || !imageName){
                Toast.show({
                    type: "error",
                    text1: "Something Went Wrong",
            ...
            const result = await firestore().collection("Posts").doc(id).update({
                "title": title,
            ...
            const fileRef = storage().ref().child(`/Posts/${id}/${imageName}`);
            ...
            () => {
                ...
                const userDocRef = firestore().collection("Posts").doc(id);
                userDocRef.update({
    ...
    return (
        <SafeAreaView style={theme == "light" ? lightStyles.SafeAreaView : darkStyles.SafeAreaView}>
            ...
            <View style={theme == "light" ? lightStyles.formArea : darkStyles.formArea}>
                ...
                <Pressable style={theme == "light" ? lightStyles.title : darkStyles.title}><Text style={theme == "light" ? lightStyles.titleText : darkStyles.titleText}>Edit Post</Text></Pressable>
                ...
                <View>
                    ...
                    <TextInput type='text' placeholder='Enter Post URL' placeholderTextColor="#C0C0C0" style={theme == "light" ? lightStyles.input : darkStyles.input} value={postUrl} onChangeText={(text) => setPostUrl(text)} />
