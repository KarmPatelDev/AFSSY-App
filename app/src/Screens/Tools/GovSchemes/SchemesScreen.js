...
export default SchemesScreen = () => {
    ...
    const addLikedSchemes = async (id) => {
        if(user == ""){
            Toast.show({
                type: 'info',
                text1: 'Something Went Wrong',
        ...
        try{
            ...
            dispatch(changeUser({
                ...
                govSchemes: [...likedSchemes, id],
                role: user.role
            }));
            setLikedSchemes([...likedSchemes, id]);
            Toast.show({
                type: 'success',
                text1: 'Scheme Saved.',
                text2: ""
    ...
    return(<>
        <SafeAreaView style={theme == "light" ? lightStyles.SafeAreaView : darkStyles.SafeAreaView}>
            ...
            <Pressable style={theme == "light" ? lightStyles.title : darkStyles.title}><Text style={theme == "light" ? lightStyles.titleText : darkStyles.titleText}>Gov. Schemes</Text></Pressable>
                        ...
                        schemes && schemes.map((scheme) => {
                            ...
                            <Pressable key={scheme.id} style={theme == "light" ? lightStyles.schemeContainer : darkStyles.schemeContainer}>
                                <View style={theme == "light" ? lightStyles.scheme : darkStyles.scheme}>
                                    ...
                                    <View>
                                        {
                                            user && likedSchemes.includes(scheme.id) ?
                                            <AntDesign name="heart" size={25} color="#e60000" onPress={() => removeLikedSchemes(scheme.id)} />
                                            :
                                            theme == "light" ? <AntDesign name="hearto" size={25} color="#000" onPress={() => addLikedSchemes(scheme.id)} /> : <AntDesign name="hearto" size={25} color="#FFF" onPress={() => addLikedSchemes(scheme.id)} />
